...
const handleResearchAreasUpdate = (newAreas) => {
	userInfo.researchAreas = newAreas
	// 同时更新本地存储
	const storedUserInfo = uni.getStorageSync('userInfo') || {}
	storedUserInfo.researchAreas = newAreas
...
onMounted(() => {
	// 从本地存储加载用户信息
	const storedUserInfo = uni.getStorageSync('userInfo')
	if (storedUserInfo && storedUserInfo.researchAreas) {
		userInfo.researchAreas = storedUserInfo.researchAreas
	}
	
	// 监听研究方向更新事件
	uni.$on('researchAreasUpdated', handleResearchAreasUpdate)
})

onUnmounted(() => {
	// 取消事件监听
	uni.$off('researchAreasUpdated', handleResearchAreasUpdate)
})
